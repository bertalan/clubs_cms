User: sembra che in questo cms manchino le pagine dedicate ai luoghi, che geolocalizzati e con il giusto profilo schema, assume sembianze semantiche precise: un moto club (organizzazione), un monumento, una piazza, un percorso turistico... etc.
siamo in grado di progettare qualcosa di simile? mi faresti un piano di integrazione?

GitHub Copilot: # Piano di Integrazione: App `places` — Luoghi Geolocalizzati con Schema.org
...
3. **Integrazione con il resto del CMS** (eventi in un luogo, club associato ha una sede, percorsi come sequenze di luoghi)
...
(function() {
    ...
    {% if place and place.latitude and place.longitude %}
        ...
        L.marker([lat, lng]).addTo(map).bindPopup("{{ place.title|escapejs }}");
...
- [ ] `startapp places`, registrazione in `INSTALLED_APPS`
- [ ] Modelli: `PlacePage`, `PlaceIndexPage`, `PlaceGalleryImage`, `PlaceTag`
- [ ] Migrazioni
- [ ] Template base + mappa singolo luogo (Leaflet + OSM)
- [ ] Schema.org JSON-LD per `PlacePage`
- [ ] Test modelli + schema

### Fase 2 — Percorsi (1-2 giorni)
- [ ] Modelli: `RoutePage`, `RouteStop`
- [ ] Template percorso con mappa multi-marker + polyline
- [ ] Upload e parsing GPX (opzionale, visualizzazione traccia)
- [ ] Schema.org `TouristTrip`
- [ ] Test percorsi

### Fase 3 — Indice e Mappa Globale (1-2 giorni)
- [ ] `PlaceIndexPage` con mappa cluster
- [ ] Filtri per tipo, città, tag
- [ ] API endpoint GeoJSON per caricamento asincrono
- [ ] Paginazione + SEO

### Fase 4 — Integrazione CMS (1 giorno)
- [ ] FK `venue` su `EventPage`
- [ ] Widget di selezione luogo nell'admin eventi
- [ ] Sidebar "Eventi in questo luogo" nella `PlacePage`
- [ ] Sidebar "Sede" nella pagina club

### Fase 5 — CSS 6 Temi (1 giorno)
- [ ] Componenti in `base.css`: `.place-card`, `.place-map`, `.route-profile`
- [ ] Override nei 6 temi per colori marker, stili card, badge tipo
- [ ] Dark mode per componente mappa

### Fase 6 — i18n e Raffinamento (1 giorno)
- [ ] `makemessages` per IT/EN/DE/FR/ES
- [ ] URL in `i18n_patterns`: `/it/luoghi/`, `/en/places/`, ecc.
- [ ] Sitemap XML con geolocalizzazione (`<geo:geo>`)
- [ ] Open Graph tags con geo-meta (`og:latitude`, `og:longitude`)

---

## Dipendenze Aggiuntive

Nessuna dipendenza pesante richiesta. Tutto con:
- **Leaflet.js** (CDN, 40kb) — mappe
- **OpenStreetMap** — tile gratuiti
- **Schema.org JSON-LD** — generato server-side in Python

Opzionali:
- `Leaflet.markercluster` — se molti punti nella mappa globale
- `django-gpx` o parsing manuale XML — se serve importazione tracce GPX

---

## Vuoi procedere?

Posso iniziare dalla **Fase 1**: creare l'app, i modelli, le migrazioni, i template base e i test. Dimmi se vuoi modifiche al piano (es. tipi aggiuntivi, campi diversi, priorità diverse).
